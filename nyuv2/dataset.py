"""
Code directly modified from : https://github.com/xapharius/pytorch-nyuv2
"""
...
class NYUv2(Dataset):
    ...
    def __init__(
        ...
        self.bilinear_transform = transforms.Compose([transforms.Resize(im_size, interpolation=Image.BILINEAR), transforms.ToTensor()])
        self.nearest_transform = transforms.Compose([transforms.Resize(im_size, interpolation=Image.NEAREST), transforms.ToTensor()])
    ...
    def __getitem__(self, index: int):
        ...
        img = self.bilinear_transform(img)
        rgb = img

        # Semantic segmentation labels
        random.seed(seed)
        seg = Image.open(os.path.join(folder("seg13"), self._files[index]))
        seg = self.nearest_transform(seg)[0]
        if isinstance(seg, torch.Tensor):
            # ToTensor scales to [0, 1] by default
            seg = (seg * 255).long()
        h,w = seg.shape
        img = torch.FloatTensor(14,h,w)
        img.zero_()
        img.scatter_(0, seg.unsqueeze(0), 1)
...
def _create_depth_files(mat_file: str, root: str, train_ids: list):
    ...
    for i in range(len(depths)):
        arr = (depths[i] * 1e4).astype(np.uint16).T
