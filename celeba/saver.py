...
class Saver():
    ...
    def log(self, model, epoch, n_iter, train_metrics, val_metrics, test_metrics, train_loss, val_loss, test_loss, optimizer=None):
        """
        ...
        for k in range(len(self.metrics)):
            iter_dict[self.metrics[k]] = {'train': float(train_metrics[k]), 'val': float(val_metrics[k]), 'test': float(test_metrics[k])}
            
        # Loss
        iter_dict['loss'] = {'train': float(train_loss), 'val': float(val_loss), 'test': float(test_loss)}
        ...
        if self.trace_param:
            ...
            iter_dict['distance_ratio'] = {'train': float(model.abs_distance_L2.numpy()/tot_distance),
                                           ...
                                           'test': float(model.abs_distance_L2.numpy()/tot_distance)}
    ...
    def checkpoint(self, model, epoch, n_iter, iter_dict, optimizer=None):
        ...
        # Saves best avg_error, if reached
